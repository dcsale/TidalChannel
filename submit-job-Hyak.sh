...
mkdir ./cases
cp --no-clobber initial.sim ./cases/runs_$starSimFile.sim
cd ./cases/
#rm log.*
#rm *.sim~
...
# on Hyak
starccm+ -batch $starMacros -np ${PBS_NP} -machinefile ${PBS_NODEFILE} -licpath 1999@mgmt2.hyak.local -batch-report runs_$starSimFile.sim 2>&1 | tee log.$starSimFile
# on local
#starccm+ -batch $starMacros -np 16 -licpath 1999@lmas.engr.washington.edu -batch-report runs_$starSimFile.sim 2>&1 | tee log.$starSimFile
