// STAR-CCM+ macro: physics_VirtualDisks_1DM.java
...
public class physics_VirtualDisks_1DM extends StarMacro {
    ...
    // File f = new File("../inputs/" + simpleAnnotation_00.getText() + ".csv");    
      ...
      List<String>	textline 		= new ArrayList<String>();
        ...
        try {
            ...
            Logger.getLogger(physics_VirtualDisks_1DM.class.getName()).log(Level.SEVERE, null, ex);
        } // end try
...
// compute the flow direction, and get a unit vector
...
// static final double flow_Umag = Math.sqrt( userFieldFunction_1**2 + userFieldFunction_2**2 + userFieldFunction_3**2 );
double flow_Umag = 1.0;
// double flow_nx   = userFieldFunction_1 + flow_Umag;
// double flow_ny   = userFieldFunction_2 + flow_Umag;
// double flow_nz   = userFieldFunction_3 + flow_Umag;
double flow_nx   = Double.parseDouble(userFieldFunction_1.getDefinition()) + flow_Umag;
double flow_ny   = Double.parseDouble(userFieldFunction_2.getDefinition()) + flow_Umag;
double flow_nz   = Double.parseDouble(userFieldFunction_3.getDefinition()) + flow_Umag;
...
// CREATE CYLINDER FOR THE INFLOW PLANE
...
// 		// CYLINDER create the Shape ParT
	    ...
	    // // create cylinder for the VDM Inflow Plane
		   //  // CYLINDER create the Shape ParT
		   //  // NOTE: should use the Laboratory coordinate system
		   //  SimpleCylinderPart simpleCylinderPart_00 = 
		   //    meshPartFactory_0.createNewCylinderPart(simulation_0.get(SimulationPartManager.class));
		   //    simpleCylinderPart_00.setPresentationName("inflow refine cylinder (" + names.get(i) + ")");
		      
		   //  // simpleCylinderPart_00.setCoordinateSystem(cartesianCoordinateSystem_1);

		   //  // start coordinate
		   //  Coordinate coordinate_55 = 
		   //    simpleCylinderPart_00.getStartCoordinate();
			  //   coordinate_55.setCoordinateSystem(cartesianCoordinateSystem_1);

			  //   coordinate_55.setCoordinate(units_2, units_2, units_2, new DoubleVector(new double[] {0.0, 0.0, 1.0}));

			  //   // coordinate_55.setValue(new DoubleVector(new double[] {0.0, 0.0, (-2.0*rotor_radius[i] - rotor_thick[i])}));
			  //   coordinate_55.setValue(new DoubleVector(new double[] {x[i]-flow_nx*rotor_radius[i], y[i]-flow_ny*rotor_radius[i], z[i]-flow_nz*rotor_radius[i]}));
			  //   coordinate_55.setValue(new DoubleVector(new double[] {x[i], y[i], z[i]}));

		   //  // end coordinate
		   //  Coordinate coordinate_66 = 
		   //    simpleCylinderPart_00.getEndCoordinate();

			  //   coordinate_66.setCoordinateSystem(cartesianCoordinateSystem_1);

			  //   coordinate_66.setCoordinate(units_2, units_2, units_2, new DoubleVector(new double[] {0.0, 0.0, 0.0}));

			  //   // coordinate_66.setValue(new DoubleVector(new double[] {0.0, 0.0, (-2.0*rotor_radius[i] + rotor_thick[i])}));
			  //   // coordinate_55.setValue(new DoubleVector(new double[] {x[i]-flow_nx*rotor_radius[i]*0.5, y[i]-flow_ny*rotor_radius[i]*0.5, z[i]-flow_nz*rotor_radius[i]*0.5}));
			  //   coordinate_55.setValue(new DoubleVector(new double[] {x[i]+10, y[i]+10, z[i]+10}));

		   //  // radius
		   //  simpleCylinderPart_00.getRadius().setUnits(units_2);

		   //  simpleCylinderPart_00.getRadius().setValue(1.0*rotor_radius[i]);

		    
		   //  // misc. options
		   //  simpleCylinderPart_00.setDoNotRetessellate(true);
		   //  // simpleCylinderPart_0.setDoNotRetessellate(false);
		   //  // simpleCylinderPart_0.getTessellationDensityOption().setSelected(TessellationDensityOption.MEDIUM);

		   //  simpleCylinderPart_00.rebuildSimpleShapePart();
